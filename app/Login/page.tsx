...
function page() {
  ...
  return (
    <Layout>
      {loading ? <div className='flex py-12 max-h-14 w-full justify-center'>
        <CircularProgress />
      </div> :
        !registerForm ?
        /*================================ Login Form ==============================*/
          ...
          <div className='w-1/2 flex flex-col shadow-lg justify-center mx-auto my-8'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='w-full flex flex-col p-8'>
              <input placeholder='نام کاربری' {...register("userName", { required: true, maxLength: 20 })} className='p-2 my-2 rounded-lg w-10/12 mx-auto border' />
              ...
              {errors.email && <span className='text-red-500 text-xs text-center w-full block'> {errors.email.message}</span>}

              <input placeholder='پسورد' {...register("password", { required: true, maxLength: 20 })} type='password' className='p-2 my-2 rounded-lg w-10/12 mx-auto border' />
